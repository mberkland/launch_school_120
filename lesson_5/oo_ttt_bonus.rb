class Board
  ...
  def incomplete_line?(line)
    to_marker_arr(@squares.values_at(*line)).include?(" ")
  end
  ...
  def winning_marker
    WINNING_LINES.each do |line|
      unless incomplete_line?(line)
        if player_won?(line)
          return find_winning_marker(line)
        end
      end
    end
    nil
  ...
  def game_winning_squares(marker) # returns an array
    winning_squares = []
    WINNING_LINES.each do |line|
      if two_squares_marked?(line, marker) && incomplete_line?(line)
...
class TTTGame
  ...
  def computer_moves
    if board.winning_squares?(computer.marker)
